...
def exp(rv):
    u = normalized(rv)
    theta = l2norm(rv)
    R = np.array([[np.cos(theta)+np.power(u[0],2)*(1-np.cos(theta)), u[0]*u[1]*(1-np.cos(theta))-u[2]*np.sin(theta), u[0]*u[2]*(1-np.cos(theta))+u[1]*np.sin(theta)],
 [u[1]*u[0]*(1-np.cos(theta))+u[2]*np.sin(theta), np.cos(theta)+np.power(u[1],2)*(1-np.cos(theta)), u[1]*u[2]*(1-np.cos(theta))-u[0]*np.sin(theta)],
 [u[2]*u[0]*(1-np.cos(theta))-u[1]*np.sin(theta), u[2]*u[1]*(1-np.cos(theta))+u[0]*np.sin(theta), np.cos(theta)+np.power(u[2],2)*(1-np.cos(theta))]])
    return R
