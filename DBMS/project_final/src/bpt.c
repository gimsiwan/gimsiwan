...
log_frame * log_buffer_array = NULL;
...
void log_flush(void){
	...
	for(i=0;i<inlog;i++){
		if(pwrite(log_fd,&log_buffer_array[i],LOGBUFSIZE,SEEK_END)<0){
			perror("write error");
			exit(-1);
		}
	}
...
int begin_transaction(void){
	...
	in_tran = true;
	begin_log.lsn = ++lsn;
	begin_log.tran_id = ++tran_id;
	begin_log.type = BEGIN;


	log_buffer_array[inlog++] = begin_log;
	
	if(inlog==log_buf_max) log_flush();

	return 0;
}

int commit_transaction(void){
	log_frame commit_log;

	commit_log.lsn = ++lsn;
	commit_log.tran_id = tran_id;
	commit_log.type = COMMIT;

	log_buffer_array[inlog++] = commit_log;
...
int abort_transaction(void){
	...
	while(1){
		...
		i = read_log(offset, log_buf_max-1);

		if(log_buffer_array[i].type = BEGIN && log_buffer_array[i].tran_id == tran_id) break;
		else if(log_buffer_array[i].tran_id == tran_id && log_buffer_array[i].type == UPDATE){

			log_frame compensation;
			off_t leaf_offset;
			page leaf_page;
			int k, index;

			leaf_offset = find_leaf(log_buffer_array[i].table_id, log_buffer_array[i].key);

			if(leaf_offset !=0 && find(log_buffer_array[i].table_id,log_buffer_array[i].key)!= NULL){
					
				index = readpage(log_buffer_array[i].table_id, leaf_offset, 1);
				leaf_page = buffer_array[index].frame.normal_page;
			
				for(k=0;k<31;k++){
					if(leaf_page.data.records[k].key == log_buffer_array[i].key){
						compensation.lsn = ++lsn;
						compensation.tran_id = log_buffer_array[i].tran_id;
						compensation.type = UPDATE;
						compensation.table_id = log_buffer_array[i].table_id;
						compensation.key = log_buffer_array[i].key;
						strcpy(compensation.old_image,log_buffer_array[i].new_image);
						strcpy(compensation.new_image,log_buffer_array[i].old_image);
						log_buffer_array[inlog++] = compensation;
						if(inlog == log_buf_max - 1) log_flush();

						strcpy(leaf_page.data.records[k].value,compensation.new_image);
						leaf_page.header.lsn = compensation.lsn;
						buffer_array[index].is_dirty = 1;
						buffer_array[index].ref_bit = 1;
						if(buffer_array[index].pin_count>0) buffer_array[index].pin_count--;
						break;
					}
				}
	...
	memset(&log_buffer_array[i],0,sizeof(log_frame));
	abort.lsn = ++lsn;
	abort.tran_id = tran_id;
	abort.type = ABORT;

	log_buffer_array[inlog++] = abort;
...
int update(int table_id, int64_t key, char * value){
	...
	if(in_tran){
		...
		if(leaf_offset !=0 && find(table_id,key)!= NULL){
			index = readpage(table_id, leaf_offset, 1);
			leaf_page = buffer_array[index].frame.normal_page;

			for(i=0;i<leaf_page.header.key_number;i++){
				if(leaf_page.data.records[i].key == key){
					log.lsn = ++lsn;
					log.tran_id = tran_id;
					log.type = UPDATE;
					log.table_id = table_id;
					log.key = key;
					strcpy(log.old_image,leaf_page.data.records[i].value);
					strcpy(log.new_image,value);
			
					log_buffer_array[inlog++] = log;
					if(inlog == log_buf_max) log_flush();				
					strcpy(leaf_page.data.records[i].value, value);
					leaf_page.header.lsn = lsn;
					buffer_array[index].frame.normal_page = leaf_page;
					buffer_array[index].is_dirty = 1;
					buffer_array[index].ref_bit = 1;
					if(buffer_array[index].pin_count>0) buffer_array[index].pin_count--;
					break;
...
int read_log(off_t offset, int index){
	...
	if(pread(log_fd,&log_buffer_array[i],LOGBUFSIZE,offset)<0){
		perror("read error");
		exit(EXIT_FAILURE);
	}
	if(inlog==log_buf_max) inlog = 0;
...
int init_db (int num_buf){
	...
	log_buffer_array = (log_frame *)calloc(1,sizeof(log_frame)*log_buf_max);

	if(log_buffer_array == NULL || buffer_array == NULL){
		perror("buffer array alloc error");
		return -1;
	}
	if((log_fd = open("log",O_RDWR|O_CREAT|O_EXCL,0777))>0){
	}
	else if((log_fd = open("log",O_RDWR))>0){
		int i=0;
		int loser_count = 0;
		off_t end_offset = lseek(log_fd, 0, SEEK_END);
		off_t offset=0;

		if(end_offset == 0) return 0;

		list * loser_tran = (list *)malloc(sizeof(list));
		loser_tran->tran_num = -1;
		loser_tran->next = NULL;

		while(offset<end_offset){
			i = read_log(offset, 0);
			if(log_buffer_array[i].type==UPDATE){
				...
				if(leaf_offset !=0 && find(log_buffer_array[i].table_id,log_buffer_array[i].key)!= NULL){
					...
					if(leaf_page.header.lsn<log_buffer_array[i].lsn){
						for(j=0;j<31;j++){
							if(leaf_page.data.records[j].key == log_buffer_array[i].key){
								strcpy(leaf_page.data.records[j].value,log_buffer_array[i].new_image);
								leaf_page.header.lsn = log_buffer_array[i].lsn;
								buffer_array[index].frame.normal_page = leaf_page;
								buffer_array[index].is_dirty = 1;
								buffer_array[index].ref_bit = 1;
								if(buffer_array[index].pin_count>0) buffer_array[index].pin_count--;
			...
			}else if(log_buffer_array[i].type == BEGIN){

				list * temp = loser_tran;

				while(temp->next != NULL){
					temp = temp->next;
				}
				temp->next = (list*)malloc(sizeof(list));
				temp->next->tran_num = log_buffer_array[i].tran_id;
				temp->next->next = NULL;
				loser_count++;

			}else if(log_buffer_array[i].type == COMMIT || log_buffer_array[i].type == ABORT){

				list * temp = loser_tran;
				while(temp->next){
					if(temp->next->tran_num == log_buffer_array[i].tran_id){
						list * have_to_free = temp->next;
			...
			}else if(log_buffer_array[i].type == OPEN){
				char path[10];
				int t_id = log_buffer_array[i].table_id;
				sprintf(path,"data%d",t_id);
				open_table(path);
			}
		...
		lsn = log_buffer_array[i].lsn;
		offset = end_offset - LOGBUFSIZE;
		...
		while(offset>=0){
			...
			if(undo && log_buffer_array[i].type == UPDATE){
				...
				if(leaf_offset !=0 && find(log_buffer_array[i].table_id,log_buffer_array[i].key)!= NULL){
					...
					if(leaf_page.header.lsn>=log_buffer_array[i].lsn){
						for(k=0;k<31;k++){
							if(leaf_page.data.records[k].key == log_buffer_array[i].key){
								strcpy(leaf_page.data.records[k].value,log_buffer_array[i].old_image);
								leaf_page.header.lsn = log_buffer_array[i].lsn-1;
								buffer_array[index].is_dirty = 1;
								buffer_array[index].ref_bit = 1;
								if(buffer_array[index].pin_count>0) buffer_array[index].pin_count--;
								break;
		...
		for(i=0;i<loser_count;i++){
			list * have_to_free = loser_tran->next;
			loser_tran->next = loser_tran->next->next;
			free(have_to_free);
		}
		free(loser_tran);

		memset(log_buffer_array,0,sizeof(log_frame)*log_buf_max);
		inlog = 0;
	}
	int index;

	for(index=1;index<11;index++){
		if(fd[index]!=0) close_table(fd[index]);
	}
	return 0;
...
int shutdown_db(void){
	...
	//log_fd = open("log",O_RDWR|O_CREAT|O_TRUNC,0777);
	close(log_fd);
...
int open_table (char *pathname){
	...
	else if((fd[table_id] = open(pathname,O_RDWR))>0){
